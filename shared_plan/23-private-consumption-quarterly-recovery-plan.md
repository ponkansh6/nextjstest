...
- 責務分離: 数値検証（原値・正規化係数・四半期結合・表/CSV/tooltipの表示値）と描画形式検証（2017Q4以前のGDP単独棒、2018Q1以降のCTI積み上げ棒、GDP線なし、GDP非加算、欠損区間の切断）を別の受入条件・証跡として記録する。前者の成功だけでは後者の完了根拠にしない。
...
修正に問題がある場合も、既知の年次反復表示へ無言で戻さない。2018Q1以降はGDP系列を消費チャートへ表示せず、CTI表示を維持したうえで結合処理を修復する。今回のPlan24で、チャート・tooltip・凡例と表・CSVの公開範囲を分離して記録する。今回コミット・プッシュは行わない。
...
- [ ] 描画形式を数値検証と分離して検証し、2017Q4以前のGDP単独棒、2018Q1以降のCTI積み上げ棒、GDP線なし、GDP非加算、欠損区間の切断を確認した（Plan24で実施）
  - 達成根拠: Plan24の描画修正・視覚検証が未実施のため、数値検証の完了を描画形式の完了とは扱わない
...
- [ ] 実データの原値・係数・計算式・期待値を独立した手順で再計算し、独立性の意味と限界を記録する
  - 達成根拠: [記入]
- [ ] 数値経路の検証と描画形式の検証を別責務として記録し、数値一致を棒・線分離やGDP非加算の根拠にしていない
  - 達成根拠: Plan24の実ブラウザー描画検証待ち
