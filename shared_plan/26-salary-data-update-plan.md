...
- `tests/e2e/advanced-series.e2e.spec.ts` は明示した8ファイルを常に読み込み、tracked/untracked状態や `git diff HEAD` に依存せず、パスと内容bytesを順序固定でhash化する。raw fixture `tests/fixtures/csv/minkan-extension-raw.csv` と anchor `tests/fixtures/minkan-extension-anchors.json` も常時含む。
- `artifacts/new-graph-default.json` / `new-graph-advanced.json`: `<!-- PLAN26_GENERATED_DIFFHASH --> ca0c5ff811bb4dd2044001eebaa3606a3b693bb2c4058f9fa8051e25d0f49472`、`head=2072cffe22de9384c9e6dda398fdb98d394f553c`。両証跡のhash対象は同じ ordered 8ファイルで一致し、URLはそれぞれ `http://127.0.0.1:3100/` / `http://127.0.0.1:3100/?adv=1`。capture timestamp・axis ticks・対象pathごとの実座標（全x min/max、subpath区間、始点終点、コマンド種別・絶対x範囲・subpath所属を含む全コマンド区間）・対象一覧を記録した。
