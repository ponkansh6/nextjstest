...
# 最終検証結果（2026-08-17 / 対象コミット `aebdd7d` + `a969a23`）

## 総合判定: **J-1〜J-4 すべて解消。全ゲート pass。追加の要修正なし。**

実ブラウザでの挙動確認も含め、本プランの目的（CAGR 設定のボトムシート化）と
その過程で派生した回帰（D-1）はすべて解決した。**残るのは J-5（実機目視確認）のみ。**
...
| ----------------------- | ------------------------------------------------------------------------------------------ |
| `pnpm type-check`       | ✅ エラー 0                                                                                |
| `pnpm lint`             | ✅ エラー 0 / **警告 5**（J-1 解消により既存分のみへ復帰。新規警告ゼロ）                   |
| `pnpm test`             | ✅ 31 ファイル / **248 テスト全 pass**（246 → T13/T14 の +2）                              |
| `pnpm build`            | ✅ 成功                                                                                    |
| `pnpm test:e2e`（全体） | ✅ **87 passed / 22 skipped / 0 failed**（85 → フォーカス管理 +1 テスト × 2 プロジェクト） |

## J 項目の解消確認

| 項目    | 解消状況                                                                                                                                                                                    |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **J-1** | ✅ `useFocusTrap.ts:39-40` でエフェクト内に `restoreTarget` をキャプチャし、クリーンアップではそれを使う。`react-hooks/exhaustive-deps` 警告は消え、**警告数は 6 → 5 の既存分のみに戻った** |
| **J-2** | ✅ `ChartInfoButton.test.tsx:287-310` に T13（✕ クローズ）/ T14（Escape クローズ）を追加。いずれも `document.activeElement` がトリガーへ戻ることを検証                                      |
| **J-3** | ✅ spec.md に 3 点すべて反映 — `:289-295` に Scenario R8e、`:509` の `src/hooks/` 表に `useFocusTrap.ts`、`:568-570` の `accessibility.e2e.spec.ts` 行にフォーカス管理を追記                |
| **J-4** | ✅ `aebdd7d` / `a969a23` の 2 コミットに分割してコミット済み。作業ツリーはクリーン（本プランと無関係な `AGENTS.md` / `shared_plan/11-*` を除く）                                            |
| **J-5** | ⏳ 未実施（下記参照）                                                                                                                                                                       |

**G 案の撤回について**: 中間コミット `aebdd7d` では `restoreFocus: false`（＝フォーカス復帰を諦める G 案）
が入っていたが、`a969a23` で撤回され `useFocusTrap(popupRef, open, triggerRef)` の
`restoreRef` 一本に統一された。**この判断は正しい。** G 案は「`preventScroll` では防げない」という
誤った前提から出た案であり、実際には `preventScroll: true` で解決するため、
アクセシビリティ上必要なフォーカス復帰を捨てる必要はなかった。

## 実ブラウザでの挙動確認（一時スペックで実測・確認後に削除済み）

E2E のアサーションだけでは「スクロールが動かない」ことしか担保されないため、
**フォーカス復帰とスクロール保持が同時に成立しているか**を production ビルドで直接観測した。

| 経路                                | scrollY 変化     | クローズ後の `document.activeElement`    |
| ----------------------------------- | ---------------- | ---------------------------------------- |
| ⓘ ポップアップ — 外側クリック       | 3000 → 3000（0） | クリック先の要素（`<g>`）                |
| ⓘ ポップアップ — ✕ クリック         | 3000 → 3000（0） | ⓘ トリガーボタン ✅                      |
| ⓘ ポップアップ — Escape             | 3000 → 3000（0） | ⓘ トリガーボタン ✅                      |
| 表示期間シート — Escape             | 2500 → 2500（0） | 「表示期間を変更」ボタン ✅              |
| CAGR シート — Escape                | 2319 → 2319（0） | CAGR トリガーボタン ✅                   |
| 表示期間 / CAGR シート — オープン時 | —                | シート内の ✕ ボタン ✅（初期フォーカス） |

- **外側クリック時にフォーカスがトリガーへ戻らないのは正常。** クリーンアップの
  `restoreTarget.focus({ preventScroll: true })` の直後に、ブラウザ既定の mousedown フォーカスが
  クリック先へ移るため。ユーザーが明示的に触った要素へフォーカスが乗る方が自然な挙動であり、
  かつ `preventScroll` によりスクロールは一切動かない。
- CAGR トリガーの `activeElement` の `aria-label` が
  `CAGRの期間・評価月を変更（現在: 2005年01月から2026年01月）` と読めることも確認でき、
  **A-2（現在値の読み上げ）が production ビルドで機能している**ことの裏付けになった。

### 計測上の注意（誤検知の記録）

初回の計測で「✕ クリックで 3000 → 193（-2807px）」という結果が出たが、これは
**Playwright の `locator.click()` がクリック前に対象要素を `scrollIntoViewIfNeeded` する**ことによる
テスト側の副作用だった。`page.evaluate()` から DOM の `element.click()` を直接呼んで再計測したところ
差分 0px・フォーカスはトリガーへ復帰、で正常だった。**画面外の要素に対する scroll 計測では
`locator.click()` を使わないこと。**

## 残件

### J-5【低優先・継続】実機幅での目視確認

プラン 63 行の「`position: fixed` のボトムシート表示位置を実機幅で目視確認」は依然として未実施。
E2E で可視性・背景タップ・横スクロールなし（T-E2E-1 / T-E2E-3 / T-E2E-4）は担保されているため
機能面のリスクは低いが、セーフエリア（iOS のホームインジケーター領域）との重なりなど、
自動テストで拾えない見た目の問題が残っている可能性はある。

### K-1【任意・テスト設計メモ】スクロール保持テストは復帰そのものを担保しない

`accessibility.e2e.spec.ts:172-193` の「スクロール位置が保持される」は、
**フォーカス復帰が完全に無効化されても pass する**（何も focus しなければスクロールも動かないため）。
D-1 の回帰検知としては十分だが、復帰そのものは jsdom の T13 / T14 が担保している、という
役割分担になっている。E2E 側でも復帰を押さえたい場合は、Escape 経路で
`document.activeElement` がトリガーであることを追加アサートするとよい（1 行）。
**現状の組み合わせで抜けは無いため、対応は任意。**
