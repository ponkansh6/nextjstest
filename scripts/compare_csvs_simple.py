...
def compare_csv(head_path, work_path, filename):
    ...
    try:
        ...
        # For working tree version, try to read with error handling
        # First, let's check the file structure
        with open(work_path, 'r', encoding='utf-8') as f:
            work_content = f.read()
        
        # Try to parse the working tree CSV
        # The issue is that the working tree CSV has multi-line headers
        # Let's try to read it with different approaches
        
        # Approach 1: Try to read with pandas directly
        try:
            ...
            # If pandas reading fails, try to read the file manually
            result["work_error"] = str(e)
            result["all_values_match"] = False
