...
def count_header_lines(filepath):
    """Count the number of header lines in a CSV file."""
    ...
    for line in lines:
        ...
        if not line:
            continue
        # Check if it's a header line (contains non-numeric data or special patterns)
        if (line.startswith(',') or line.startswith('年') or line.startswith('Unnamed') or
            line.startswith('毎月') or line.startswith('　') or
            any(char in line for char in ['年', '月', '日', '年平均', '上半期', '下半期'])):
            header_lines += 1
        else:
            # If we hit a data line (starts with a number), stop counting
            break
...
def compare_csv(head_path, work_path, filename):
    ...
    result = {
        ...
        "head_header_lines": 0, "work_header_lines": 0,
    ...
    try:
        # Count header lines
        result["head_header_lines"] = count_header_lines(head_path)
        result["work_header_lines"] = count_header_lines(work_path)
        
        # Read CSV files with proper handling of multi-line headers
        # For HEAD version, use standard pandas reading
        head_df = pd.read_csv(head_path, dtype=str)
        
        # For working tree version, we need to handle multi-line headers
        # First, let's read the raw file to understand the structure
        ...
        for i, line in enumerate(work_lines):
            ...
            if not line:
                continue
            # Check if it's a data line (starts with a number or has fewer commas than header)
        ...
        # Read the working tree CSV starting from the data
        work_df = pd.read_csv(work_path, dtype=str, skiprows=work_data_start-1)
...
for f in FILES:
    ...
    # Print summary
    print(f"  HEAD rows: {result['head_rows']}, cols: {result['head_cols']}, header lines: {result['head_header_lines']}")
    print(f"  WORK rows: {result['work_rows']}, cols: {result['work_cols']}, header lines: {result['work_header_lines']}")
...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    for r in results:
        ...
        f.write(f"- {status} {r['file']}: HEAD={r.get('head_rows','?')}r/{r.get('head_cols','?')}c ({r.get('head_header_lines','?')} header lines) vs WORK={r.get('work_rows','?')}r/{r.get('work_cols','?')}c ({r.get('work_header_lines','?')} header lines) {diff_info}\n")
    ...
    for r in results:
        ...
        f.write(f"- HEAD rows: {r['head_rows']}, columns: {r['head_cols']}, header lines: {r['head_header_lines']}\n")
        f.write(f"- WORK rows: {r['work_rows']}, columns: {r['work_cols']}, header lines: {r['work_header_lines']}\n")
