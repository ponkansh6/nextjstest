...
def read_csv_with_header_detection(filepath):
    ...
    try:
        ...
        # Find the first line that looks like data (not a header)
        data_start = 0
        ...
        if data_start > 0:
            # Read the CSV starting from the data
...
def compare_csv(head_path, work_path, filename):
    ...
    try:
        # Read HEAD CSV
        head_df = read_csv_with_header_detection(head_path)
        result["head_rows"] = len(head_df)
        result["head_cols"] = len(head_df.columns) if len(head_df) > 0 else 0
        
        # Read working tree CSV
        work_df = read_csv_with_header_detection(work_path)
