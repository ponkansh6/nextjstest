...
export default function CpiChart({ data }: CpiChartProps) {
  ...
  const stackedColors = [
    "#6b7a8f", // 住居 (落ち着いた青紫)
    "#7c98b3", // 家具・家事用品 (落ち着いた青)
    "#8eb9a8", // 被服及び履物 (彩度を抑えた青緑)
    "#7da8a0", // 教育 (緑がかった青緑)
    "#7dbd8e", // 交通 (交換: 自動車等関係費の色へ)
    "#5a9e70", // 自動車等関係費 (交換: 交通の色へ)
    "#a3d6a3", // 通信 (淡い緑)
    "#e6d78a", // 光熱・水道 (明るい黄)
    "#e6a970", // 保健医療 (黄色からオレンジへの接続)
    "#d48866", // 教養娯楽用品 (明るいオレンジ)
    "#c67040", // 教養娯楽サービス (オレンジ味を強調)
    "#c67070", // 外食以外食料 (交換: 外食の色へ)
    "#d98c8c", // 外食 (交換: 外食以外食料の色へ)
    "#9ba9ab", // 諸雑費 (明るい灰色)
  ];
