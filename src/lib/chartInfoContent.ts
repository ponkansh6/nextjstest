    ...
    sections: [
      {
        ...
        items: [
          ...
          {
            text: "民間最終消費支出（参考）：2005年〜2016年、四半期別GDP統計の「民間最終消費支出（名目原系列）」を12か月移動平均で指数化",
          },
          {
            text: "CTI消費支出（参考）：2017年以降、分布調整済み原数値CTIの「消費支出（名目）」を12か月移動平均で指数化（※物差しが異なるため両系列を直接比較することはできません）",
          },
