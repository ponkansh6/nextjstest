    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            text: "GDP参考値は名目・実質それぞれのraw値に係数を適用した比較指数（2025年平均=100）です。実質連鎖系列は非加法的なため、内訳を足し上げません。",
          },
          {
          ...
          {
            text: "表、CSV、ツールチップのGDP参考値は比較指数として、名目・実質と価格概念を区別して表示します。ツールチップの「合計」は、その時点で表示中（凡例で非表示にしていない）のCTI費目のみの合計値です。",
          },
        ],
      },
    ],
    ...
    sections: [
      {
        ...
        items: [
          ...
          {
            text: "民間最終消費支出（総合）：四半期別GDP統計の既存の名目・実質各1系列を、2025年平均=100の比較指数として四半期粒度で表示します（公式金額そのものではありません）。raw値と比較指数は内部で検証・保持し、年次経路の2025年年次正規化とは別管理です。独立照合がpendingの間は比較線を表示しません。",
          },
          {
