...
- **THEN** the following charts are rendered:
  ...
  - MajorIndicesChart, ResidualAreaChart, and NewGraph use the shared `TimeSeriesXAxis` component (`src/app/components/charts/TimeSeriesXAxis.tsx`) for their year/month axis. On mobile it retains all data-present milestone candidates and suppresses only candidates whose estimated SVG label rectangle intersects an endpoint; endpoint labels retain the existing centered `text-anchor="middle"` behavior. Desktop keeps the existing milestone and boundary-tick policy.
