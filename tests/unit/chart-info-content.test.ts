...
describe("new-graph chart info (3種比較)", () => {
  ...
  it("消費支出（参考）の説明が実際のデータソースと一致している", () => {
    const item = info.sections
      .flatMap((s) => s.items)
      .find((i) => i.text.startsWith("消費支出（参考）"));
    expect(item, "消費支出（参考）の説明が見つからない").toBeDefined();
    // 家計調査は使用していないため説明に登場してはならない
    expect(item!.text).not.toContain("家計調査");
    expect(item!.text).toContain("分布調整済み原数値CTI");
    expect(item!.text).toContain("四半期別GDP統計");
    expect(item!.text).toContain("民間最終消費支出");
    expect(item!.text).toContain("12か月移動平均");
  });
});
